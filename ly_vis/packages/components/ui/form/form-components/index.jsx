...
// 传输密钥 PSK 输入（密码框 + 随机生成按钮），留空=明文直连
const genPsk = () => {
...
export const PskInput = ({ rules = [], ...props }) => {
    const Cmp = withFormItem(
        forwardRef((_props, ref) => {
            ...
            return (
                <Input.Password
                    ...
                    {..._props}
                    addonAfter={
                        <Tooltip title='随机生成 48 位密钥'>
                            <RetweetOutlined
                                onClick={() => onChange && onChange(genPsk())}
        ...
        ...rules,
        {
            validator: (rule, value) => {
                if (value && value.length < 16) {
                    return Promise.reject(new Error('密钥长度至少 16 位!'))
                }
                return Promise.resolve()
            },
        },
