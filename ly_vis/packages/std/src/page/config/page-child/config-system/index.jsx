...
const deviceColumns = [
    ...
    },
    {
        title: '加密传输',
        dataIndex: 'tls_psk',
        sorter: valueSort('tls_psk'),
        render: psk =>
            psk ? (
                <span title={psk}>
                    {psk.slice(0, 4)}****{psk.slice(-4)}
                </span>
            ) : (
                <span style={{ color: '#999' }}>未配置(明文)</span>
            ),
    },
    {
        title: '连接状态',
        dataIndex: 'tls_status',
        sorter: valueSort('tls_status'),
        render: status => {
            if (!status) return <span style={{ color: '#999' }}>未启用</span>
    ...
    {
        title: '最后连接',
        dataIndex: 'tls_last_seen',
        sorter: valueSort('tls_last_seen'),
        render: t =>
            t ? (
                formatTimestamp(t)
            ) : (
                <span style={{ color: '#999' }}>从未</span>
            ),
    },
    {
        title: '禁止使用',
