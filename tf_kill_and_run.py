...
cmds = [
    ("Kill concurrent instances and add lock", r"""
echo "=== 1. 杀掉所有并发实例 ==="
pkill -9 -f "extractor -v" 2>/dev/null
pkill -9 -f "launch_indexer" 2>/dev/null
pkill -9 -x indexer 2>/dev/null
sleep 2
ps aux | grep -E "[e]xtractor|[i]ndexer|[l]aunch_indexer" | grep -v bash | head -5
echo "清理完成"
echo ""
...
echo "已加锁"
echo ""
echo "=== 3. 手动跑一次（单实例完整回放） ==="
timeout 600 /Agent/bin/launch_indexer.sh 2>&1 | tail -3
echo "Done, exit: $?"
"""),

    ("Verify db after run", r"""
echo "=== 4. db 写入结果 ==="
